...
@section('title', 'Gestión de Sellos - UNEY')

@push('styles')
    <!-- CSS Propio de esta vista -->
    <link rel="stylesheet" href="{{ asset('assets/css/coordinador/entidades_crud/sello.css') }}">
@endpush
...
<header class="main-header">
    <div class="header-welcome">
        <h1>Sellos Institucionales</h1>
        <p>Gestión de sellos húmedos y digitales para el aval de documentos académicos</p>
    </div>
...
<!-- Contenedor de Alertas Flotantes (Fijas por CSS para evitar mover la estructura HTML) -->
...
<section class="crud-container">
    ...
    <div class="table-actions-bar">
        <div class="search-wrapper">
            ...
            <input type="text" placeholder="Buscar sello..." class="search-input" id="search-sello" autocomplete="off">
        </div>
        ...
        <div class="filters-wrapper">
            ...
            <button class="btn-primary-action" id="btn-open-create">
                <i class="fa-solid fa-plus"></i> Agregar Sello
            </button>
        </div>
    </div>
        ...
        <table class="crud-table">
            ...
            <tbody id="table-body">
                ...
                <tr class="data-row" 
                    ...
                    <td><strong>{{ $registro->id_sello }}</strong></td>
                    <td>
                        <div class="table-primary-text">{{ $registro->nombre }}</div>
                        <span class="table-secondary-text">Cod: SLL-{{ str_pad($registro->id_sello, 3, '0', STR_PAD_LEFT) }}</span>
                    </td>
                    <td>
                        @if($registro->imagen)
                            <div class="table-image-wrapper">
                                <img src="{{ asset('storage/' . $registro->imagen) }}" alt="Sello {{ $registro->nombre }}" class="table-preview-img">
                            </div>
                        @else
                    ...
                    <td>
                        <div class="action-buttons">
                            <button class="btn-action-edit"
                                title="Editar"
                                onclick="openEditModal('{{ $registro->id_sello }}', '{{ addslashes($registro->nombre) }}', '{{ $registro->imagen ? asset('storage/' . $registro->imagen) : '' }}', '{{ $registro->estado ? 'active' : 'inactive' }}')">
                                <i class="fa-solid fa-pen"></i>
                            </button>
                            <button class="btn-action-delete"
                                title="Inhabilitar"
                                onclick="openDeleteModal('{{ $registro->id_sello }}', '{{ addslashes($registro->nombre) }}')">
                ...
                <tr class="empty-state-row" id="no-results-row" style="display: none;">
                    <td colspan="5" style="text-align: center; padding: 3rem; color: var(--color-text-muted);">
                        ...
                        No se encontraron sellos que coincidan con la búsqueda.
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <form id="create-form" action="{{ route('coordinador.entidades_crud.store', ['modulo' => $modulo]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <div class="form-group-row">
                    <i class="fa-solid fa-stamp input-row-icon"></i>
                    <div class="input-container">
                        <label for="create-nombre">Nombre del Sello</label>
                        <input type="text" id="create-nombre" name="nombre" placeholder="Ej. Rectorado" autocomplete="off" value="{{ old('nombre') }}" class="@error('nombre') val-red @enderror">
                        @error('nombre')
                ...
                <div class="form-group-row align-start">
                    ...
                    <div class="input-container">
                        <label for="create-imagen">Imagen del Sello (.png, .jpg, .svg)</label>
        ...
        <form id="edit-form" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <div class="form-group-row">
                    <i class="fa-solid fa-stamp input-row-icon"></i>
                    <div class="input-container">
                        <label for="edit-nombre">Nombre del Sello</label>
                ...
                <div class="form-group-row align-start">
                    <i class="fa-solid fa-image input-row-icon mt-xs"></i>
                    <div class="input-container">
                        <label for="edit-imagen">Reemplazar Imagen del Sello (Opcional)</label>
                        ...
                        <div id="edit-image-preview-container" class="image-preview-box" style="display: none; margin-top: 1rem;">
                            <span class="table-secondary-text" style="display: block; margin-bottom: 0.5rem;">Sello actual cargado:</span>
                            <img id="edit-preview" src="" alt="Sello actual" class="modal-preview-img">
                        </div>
                        @error('imagen')
                            <span class="error-input-text">{{ $message }}</span>
                        @enderror
        ...
        <form id="delete-form" method="POST">
            ...
            <div class="modal-body text-center">
                ...
                <p class="delete-warning-text">
                    ¿Está seguro de que desea inhabilitar el sello de <strong id="delete-item-name"></strong>?
                </p>
                <p class="delete-sub-text">Esta acción cambiará el estado del sello a inhabilitado.</p>
            </div>
            <div class="modal-footer">
...
@push('scripts')
    ...
    <script src="{{ asset('assets/js/coordinador/entidades_crud/sello.js') }}"></script>
@endpush