document.addEventListener('DOMContentLoaded', () => {
    ...
    window.openEditModal = function(idModalidad, nombreModalidad, estado) {
        resetStyles(editNombre);

        if (editForm) {
            // CORREGIDO: Se cambia el prefijo a 'entidades_crud'
            editForm.action = '/coordinador/entidades_crud/modalidad/' + idModalidad; 
        }

        const inputNombre = document.getElementById('edit-nombre');
        const inputEstado = document.getElementById('edit-estado');

        if (inputNombre) inputNombre.value = nombreModalidad;
    ...
    window.openDeleteModal = function(idModalidad, nombreModalidad) {
        if (deleteForm) {
            // CORREGIDO: Se cambia el prefijo a 'entidades_crud'
            deleteForm.action = '/coordinador/entidades_crud/modalidad/' + idModalidad; 
        }

        const txtName = document.getElementById('delete-item-name');
        if (txtName) {
            txtName.innerText = nombreModalidad;
        }
    ...
    // Regla de validación para el nombre de modalidad (Asistencia visual)
    ...
    // Escuchadores para inputs de nombres (Creación y Edición)
    if (createNombre) {
    ...
    const searchInput = document.getElementById('search-modalidad');
